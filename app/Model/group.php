<?php
App::uses('AppMachaModel', 'Macha.Model');
 ...
 *
 */
class Group extends AppMachaModel
{
    ...
    public function Create_Group($_data, $_userid, $error = null)
    {
        ...
        // 管理者モデルの読み込み
        App::import('Model', 'Macha.Administrator');
        $administrator = new Administrator();

        // 参加情報モデルの読み込み
        App::import('Model', 'Macha.Join');
        $join = new Join();
    ...
    {
        // ストレージモデルの読み込み
        App::import('Model', 'Macha.Storage');
        $storage = new Storage();
...
//        $login_user = $Session->read('Auth.User');
        ...
        $login_user = CakeSession::read('Auth.User');
    ...
    public function invite($_grpid, $_data)
    {
        ...
        // 申請情報モデルの読み込み
        App::import('Model', 'Macha.Request');
        $request = new Request();
        ...
        if ($request->saveAll($params)) {
            if ($join->saveAll($g_params)) {
                // コンポーネントの読み込み
                App::import('Component', 'Macha.Common');
                $common = new CommonComponent();
                // メールの用意
                $url = Router::url(array(
                    'controller' => 'Macha.users',
                    'action' => 'login'
                ), true);
    ...
    {
        // 参加情報モデルの読み込み
        App::import('Model', 'Macha.Timeline');
