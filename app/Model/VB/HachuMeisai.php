<?php
class HachuMeisai extends AppModel {

    public $useTable = false;
//    public $primaryKey = array('ID','SEIKYU_YM','MOTO_SOSHIKI_CD','SAKI_SOSHIKI_CD','SEIKYU_SIHARAI_CD','HIYOKOMOKU_CD');

    function getHachuKobaiManageJoho(){

    /*
		Public Function GetHachuKobaiManageJoho(JyobuSoshikiCd As String, ShiiresakiCd As String, KobaiBunrui As String, KobaiMei As String, KobaiCd As String, ByRef MsgCd As String, FindFlg As Integer) As Kobaihin()
			Dim masterSQL As MasterSQL = New MasterSQL()
			Dim result As Kobaihin()
			Try
				Dim flag As Boolean = Strings.Len(ShiiresakiCd) = 4
				If flag Then
					ShiiresakiCd = Strings.Trim(JyobuSoshikiCd + ShiiresakiCd)
				End If
				flag = (FindFlg = 0)
				Dim strSQL As String
				If flag Then
					strSQL = String.Format(masterSQL.SCF0421().ToString(), New Object()() { JyobuSoshikiCd, ShiiresakiCd, KobaiBunrui, KobaiMei, KobaiCd })
				Else
					strSQL = String.Format(masterSQL.SCF0422().ToString(), JyobuSoshikiCd, ShiiresakiCd)
				End If
				...
				Me.DAL.Close()
				result = array
			Catch expr_A62 As Exception
				ProjectData.SetProjectError(expr_A62)
				Dim ex As Exception = expr_A62
