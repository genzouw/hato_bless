...
/****** Object:  StoredProcedure [dbo].[MAKE_IRAIMOTO_RYOKINDATA_ETC3]    Script Date: 07/31/2014 11:36:43 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[MAKE_IRAIMOTO_RYOKINDATA_ETC3](@UKETSUKE_NO CHAR(8),@SOSHIKI_CD CHAR(5)) AS
BEGIN
	DECLARE
	...
	BEGIN
		SELECT @ERR = -99
		GOTO MAKE_IRAIMOTO_RYOKINDATA_ETC3_ERR
	END
--    SET @LOG_MSG = '依頼元組織ＣＤ='+@IRAIMOTO_SOSHIKI_CD+'を取得'
--    EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	--依頼元組織コードが登録されていない場合は処理を抜ける
	IF @IRAIMOTO_SOSHIKI_CD IS NULL
	BEGIN
		SELECT @ERR = -1
		GOTO MAKE_IRAIMOTO_RYOKINDATA_ETC3_EXIT
	END
...
--    EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	...
	BEGIN
		GOTO MAKE_IRAIMOTO_RYOKINDATA_ETC3_ERR
	END
--    SET @LOG_MSG = '料金明細を削除する='+CONVERT(VARCHAR,@ERR)
--    EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	--料金情報、料金明細を作成又は更新する。
	--また、料金モードも同じに更新する。
	--料金明細の作成
...
--    EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	SELECT	@RYOKIN_1             = RYOKIN_1,
			...
			@RYOKIN_NEBIKI_GKN    = RYOKIN_NEBIKI_GKN,			--2004.07.24
			@RYOKIN_NEBIKI_GKS    = RYOKIN_NEBIKI_GKS,			--2004.07.24
...
--    EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
MAKE_IRAIMOTO_RYOKINDATA_ETC3_EXIT:
	RETURN @ERR
MAKE_IRAIMOTO_RYOKINDATA_ETC3_ERR:
	IF @ERR = 0
		SET @LOG_MSG = '受注詳細データなし'
    ELSE
        SET @LOG_MSG = 'SQLエラー発生 Status:'+CONVERT(VARCHAR(10),@ERR)
	EXEC WRITE_PRCLOG 'RYOKINDATASP_2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'E',@LOG_MSG
	GOTO MAKE_IRAIMOTO_RYOKINDATA_ETC3_EXIT
END
