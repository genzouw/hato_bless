...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/hato_bless`](https://github.com/genzouw/hato_bless) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は **MUST NOT** です。該当する PR を確認した場合、リポジトリオーナー ([@genzouw](https://github.com/genzouw)) が内容の良し悪しにかかわらずクローズします (現時点で違反 PR を自動的にクローズする workflow / Action / Bot は存在せず、人手によるレビューで運用しています)。
...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、新規・既存を問わずワークフロー / Action に含まれていない。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **SHOULD** 取得してください。承認のない有料サービス導入 PR は、リポジトリオーナーが確認次第クローズします (自動クローズの仕組みはなく、人手によるレビューで運用しています)。
