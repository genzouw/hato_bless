...
/****** Object:  StoredProcedure [dbo].[MAKE_IRAIMOTO_RYOKINDATA_ETC2]    Script Date: 07/31/2014 11:36:26 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[MAKE_IRAIMOTO_RYOKINDATA_ETC2](@UKETSUKE_NO CHAR(8),@SOSHIKI_CD CHAR(5)) AS
BEGIN
	DECLARE
		@IRAIMOTO_SOSHIKI_CD		CHAR(5),
		@RYOKIN_MODE				CHAR(1),
		@MITSUMORI_KEITAI_KBN		CHAR(1),
		@DATA_COUNT					INT,
		@ERR						INT,
		@LOG_MSG					VARCHAR(200),
		@RYOKIN_1					DECIMAL(10),
...
--    EXEC WRITE_PRCLOG 'RYOKINDATASP2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
...
--    EXEC WRITE_PRCLOG 'RYOKINDATASP2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	--存在しない場合は料金情報を作成する。
	--存在する場合は内手数料、粗利以外の項目更新する。
	--また、料金モード、見積形態も同じに更新する。
	IF @DATA_COUNT = 0
	BEGIN
		DELETE T_JUCHU_RYOKIN
			WHERE UKETSUKE_NO = @UKETSUKE_NO
			AND   SOSHIKI_CD  = @IRAIMOTO_SOSHIKI_CD
		SELECT @ERR = @@ERROR
		IF @ERR != 0
		...
		INSERT INTO T_JUCHU_RYOKIN (
				...
				RYOKIN_SHOKEI,
				RYOKIN_NEBIKI_GK,
				--RYOKIN_TESURYO_GK,   --2004.07.25 Del By Kos 内手数料は引き継がない
				RYOKIN_KEI,
				SHOHIZEI_RT,
				SHOHIZEI_GK,
				KOJINFUTAN_GK,
...
--        EXEC WRITE_PRCLOG 'RYOKINDATASP2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'D',@LOG_MSG
	...
	BEGIN
		SELECT	@RYOKIN_1             = RYOKIN_1,
				...
				@RYOKIN_NEBIKI_GKN    = RYOKIN_NEBIKI_GKN,		--2004.07.24 Add By Kos
				@RYOKIN_NEBIKI_GKS    = RYOKIN_NEBIKI_GKS,		--2004.07.24 Add By Kos
...
MAKE_IRAIMOTO_RYOKINDATA_ETC_ERR:
	...
	EXEC WRITE_PRCLOG 'RYOKINDATASP2',@UKETSUKE_NO,@IRAIMOTO_SOSHIKI_CD,'E',@LOG_MSG
	GOTO MAKE_IRAIMOTO_RYOKINDATA_ETC_EXIT
END
