...
/****** Object:  StoredProcedure [dbo].[EB0120]    Script Date: 07/31/2014 11:34:43 ******/
SET ANSI_NULLS ON
GO
...
/****** オブジェクト :  ストアド プロシージャ dbo.EB0120    スクリプト日付 : 2004/07/13 22:01:35 ******/
/****** オブジェクト :  ストアド プロシージャ dbo.EB0120    スクリプト日付 : 2004/07/12 11:26:13 ******/
/**********************/
/* 請求データ精算確定作成処理 */
/**********************/
CREATE PROCEDURE [dbo].[EB0120]
AS
	...
	AND T_SEISAN_JUCHU.SEIKYU_SIHARAI_CD		= '1'
	/* 清算確定登録処理、明細登録処理 */
	...
	-- 20040728 ukita
	delete T_SEISAN_MEISAI WHERE SEIKYU_SIHARAI_CD = '1' AND MEISAI_KBN = '2' AND SEIKYU_YM = @清算月 AND KAKUTEI_KESHI_KBN = '0' 
    -- ローカルカーソルを開く
    Open My_cur
    -- 結果セットの行数を取得する
	...
	--end
     Set @cnt = @cnt + 1
						...
						if @Check組織コード is null or 
						  (@Check組織コード is not null and @確定消し区分 = '0')
							begin		
						 if @請求支払いコード = '1' 
						 begin
									/* 精算確定明細テーブル登録処理 */		 
